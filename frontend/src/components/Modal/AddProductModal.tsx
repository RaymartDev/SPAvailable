...
function AddProductModal({ open, onClose }: AddProductModalProps) {
  ...
  const handleProductSelect = (index: number) => {
    ...
    if (isSelected) {
      setSelectedProducts(
        selectedProducts.filter((itemIndex) => itemIndex !== index)
      );
    } else {
      ...
      <div className="flex flex-col items-center w-full h-full bg-slate-50 rounded-2xl">
        <div className="flex justify-between items-center w-full h-fit pl-10 pr-5 py-5 shadow-lg">
          <div className="flex items-end gap-x-5 sticky justify-start">
            <h1 className="text-3xl font-bold">Select Product</h1>
        ...
        <div className="overflow-y-scroll px-10 pb-10">
          <div className="grid grid-cols-4">
            {items.map((item, index) => (
              <div
                key={index}
                className={`rounded-3xl border-4 p-2 mx-4 mt-4 hover:shadow-lg ${selectedProducts.includes(index) ? 'border-[#41924B]' : ''}`}
